...
export type TTestContextHandler = (
  testCallback: () => Promise<void>,
) => Promise<void>;

export function createHookTests(
  createTitle: TCreateHookTitleFunction,
  contextHandler?: TTestContextHandler,
) {
  const runTestInContext =
    contextHandler ??
    (async (testCallback: () => Promise<void>) => await testCallback());
  describe("useComponentTitle", () => {
    it("sets given value as document.title", async () => {
      await runTestInContext(async () => {
        createTitle("mounted");
